        ...
        var allSongs = GetAllSongs();
            ...
            if (string.IsNullOrWhiteSpace(SearchText))
            {
                foreach (var song in allSongs)
                {
                    ...
                    var songID = song.ID;
                    var stringifiedID = songID.ToString();

                    if (!string.IsNullOrWhiteSpace(songName) && !string.IsNullOrWhiteSpace(stringifiedID))
                    {
            ...
            var pagedMusicList = musicList
                .Skip(SelectedPageIndex * ListingsPerPage)
                .Take(ListingsPerPage)
                .ToList();

            var guiMusicList = new GuiBindingList<string>();

            foreach (var song in pagedMusicList)
            {
                guiMusicList.Add(song);
            }

            MusicList = guiMusicList;
            MusicToggled = musicToggled;
